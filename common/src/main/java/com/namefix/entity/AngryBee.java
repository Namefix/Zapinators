...
public class AngryBee extends AbstractHurtingProjectile {
	...
	@Override
	public void tick() {
		if(!this.level().isClientSide && (this.tickCount > 200 || Utils.isEntityInLiquidCustom(this))) {
			((ServerLevel)this.level()).sendParticles(new DustParticleOptions(0xbfbfbf, 1.0f), this.getX(), this.getY(), this.getZ(), 1, 0.0D, 0.0D, 0.0D, 0.0);
			this.discard();
			return;
		}
	...
	protected void onHitEntity(EntityHitResult entityHitResult) {
		...
		if(this.getOwner() != null) {
			...
			if(target instanceof LivingEntity)
				target.hurtServer((ServerLevel) target.level(), damageSources().sting((LivingEntity) target), baseDamage);
			else
				target.hurtServer((ServerLevel) target.level(), damageSources().generic(), baseDamage);
		}
